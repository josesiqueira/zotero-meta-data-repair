...
(function () {
  ...
  const MENU_ICON = "🔧 "; // wrench emoji + space, shown left of the label
  ...
  function addDomMenu(window) {
    ...
    // Wrench emoji as the icon, rendered inline to the left of the label text.
    // (A XUL menuitem `image` only takes a file URL, so an emoji lives in the
    // label; this also dodges the context-stroke SVG-invisibility gotcha.)
    mi.setAttribute("label", MENU_ICON + "Repair metadata…");
    ...
    const onShowing = function () {
      try {
        ...
        mi.setAttribute("label", items.length > 1
          ? MENU_ICON + "Repair metadata (" + items.length + ")…"
          : MENU_ICON + "Repair metadata…");
  ...
  ZMR.runRepair = async function runRepair(items) {
    ...
    const proposals = (built || []).filter(Boolean);
    if (!proposals.length) return;

    const hasChanges = (p) =>
      !!(p && p.fields && Object.keys(p.fields).length > 0);

    // ---- SINGLE item: ALWAYS open the dialog (even with 0 changes). --------
    if (targets.length === 1) {
      ...
      try {
        win.openDialog(DIALOG_URL, "zmr-diff", FEATURES, io);
      } catch (e) {
        ZMR.log("runRepair: openDialog failed: " + e);
        return;
      }

      if (!io.out.approved) return;
      ...
      try {
        ...
        if (res && res.changed && res.changed.length) applied = res.changed.length;
      } catch (e) {
        ZMR.log("runRepair: applyApproved failed for " + proposal.itemKey + ": " + e);
      }

      ZMR.notify(applied > 0
        ? "Updated " + applied + " field(s)."
        : "No changes applied.");
      return;
    }

    // ---- BATCH: only items WITH changes get a window; rest are counted. ----
    const changed = proposals.filter(hasChanges);
    const total = proposals.length;
    const noChange = total - changed.length;
    let applied = 0;

    for (let i = 0; i < changed.length; i++) {
      const proposal = changed[i];
      const item = Zotero.Items.getByLibraryAndKey(
        proposal.libraryID, proposal.itemKey);
      if (!item) continue;
      ...
      const io = {
        ...
        itemDisplayTitle: item.getField("title") || "(no title)",
        out: { approved: false, fieldsToApply: [], overwriteNonEmpty: {} },
        batch: { index: i + 1, total: changed.length },
        Zotero,
      };
    ...
    ZMR.notify("Repaired " + applied + " of " + total + ". " +
      noChange + " had no proposed changes.");
  };
